...
{
    public class DamagedItemProfile:Profile
    {
        public DamagedItemProfile()
        {
            CreateMap<DamegedItemRequest, DamagedItem>()
                .ForMember(dest => dest.IsDeleted, obj => obj.MapFrom(src => false))
                .ForMember(dest => dest.IsActive, obj => obj.MapFrom(src => true)).ReverseMap();

            CreateMap<DamagedItem, DamagedItemResponse>().ReverseMap();
        }
    }
}
